...
class Recommender:
    ...
    def clear_purchases(self):
        #goes through all id's without missing, but hits redis 0^n times deleting
        for id in Product.objects.values_list('id', flat=True):
            r.delete(self.get_product_key(id))

        # get all recommendation keys directly from Redis
        #and deletes in ine redis call
        # keys = r.keys('recommendations:product:*')
        # if keys:
        #     r.delete(*keys)